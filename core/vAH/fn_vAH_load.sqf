...
if (count all_ah_items IsEqualTo 0) then {
		_myListbox lbAdd format ["Il n'y a pas d'objet dans l'HDV pour le moment"];
	...
	} else {
{
	...
	_myListbox lbAdd format ["%5 l'article %3 pour $%2 par %1, expire dans %4 heures",_sellerName,[_price] call life_fnc_numberText,_itemName,[_time] call life_fnc_numberText,[_amount] call life_fnc_numberText];
...
hint "Chargement des offres effectuées";
