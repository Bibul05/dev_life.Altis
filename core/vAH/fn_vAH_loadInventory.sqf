...
if (count playerInventoryArray IsEqualTo 1) then {
		_myInvListbox lbAdd format ["Vous n'avez aucun article dans votre inventaire ou dans l'HDV!"];
	} else {
{
	_status = [_x,2,3,[3]] call BIS_fnc_param;
	switch (_status) do {
		case 0: {_stat = ": AH: À vendre"}; //AH: for sale
		case 1: {_stat = ": AH: Expiré"}; //AH: expired
		case 2: {_stat = "VENDU NE DEVRAIT JAMAIS ÊTRE VISIBLE"}; //sold
	...
	if (_item IsEqualTo "yolo") then {
			_myInvListbox lbAdd format ["--------------------Listes des objets en HDV-----------------------"];
	...
	switch (_type) do {
		case "imenu": {
			...
			_time = round (_time / 3);
			if (_time <= 0) then {_price = 0; if (_status IsEqualTo 1) then {_toDel = round(_time + 216); _timeDisplay = format[": Retiré dans %1 Hrs",_toDel]} else {_timeDisplay = "";};} else {_timeDisplay = format[": Expire dans %1 Hrs",_time]};
			if (_price IsEqualTo 0) then {_priceDisplay = ""} else {_priceDisplay = format[": Prix HDV $%1",[_price]call life_fnc_numberText];};
			};
		case "ymenu": {
